...
// Mock de Agregados
const agregadosMock = [
    { nome: "MANTRAN TRANSPORTADORA", documento: "65863592000110", tipo: "CNPJ" },
    { nome: "ALAN ROBERT", documento: "84653995060", tipo: "CPF" },
];

function AgregadoSelecaoModal({ open, resultados, onSelect, onClose }) {
        ...
        <>
            ...
            >
                <div className="p-3 text-[13px]">
                    <h3 className="font-semibold text-red-700 mb-2 border-b pb-1">Selecione o Agregado</h3>
                            ...
                            <tbody>
                                {resultados.map((item, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="border px-2 py-1">{item.documento}</td>
...
export default function InputBuscaAgregado({
    label = "Agregado",
    value = "",
    onChange,
    onSelect,
    ...
    const handleKeyDown = (e) => {
        ...
        const filtrados = agregadosMock.filter((item) => {
            const matchNome = item.nome.toUpperCase().includes(termo);
            const matchDoc = termoDigits && item.documento.includes(termoDigits);
            return matchNome || matchDoc;
        });

        if (filtrados.length === 0) {
            alert("Nenhum agregado encontrado.");
            return;
        }
