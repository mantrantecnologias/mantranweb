// src/pages/VeiculoModelo.jsx
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import {
    XCircle,
    RotateCcw,
    PlusCircle,
    Edit,
    Trash2,
} from "lucide-react";
import { useIconColor } from "../context/IconColorContext";

/* Helpers */
function Label({ children, className = "" }) {
    return (
        <label className={`text-[12px] text-gray-700 flex items-center ${className}`}>
            {children}
        </label>
    );
}

function Txt(props) {
    return (
        <input
            {...props}
            className={`border border-gray-300 rounded px-1 py-[2px] h-[26px] 
      text-[13px] w-full ${props.className || ""}`}
        />
    );
}

export default function VeiculoModelo({ open }) {
    const navigate = useNavigate();
    const { footerIconColorNormal, footerIconColorHover } = useIconColor();

    /* 👉 MOCK DOS MODELOS */
    const mock = [
        { codigo: "001", descricao: "TRACAO" },
        { codigo: "002", descricao: "IMPLEMENTO" },
        { codigo: "003", descricao: "XRE 300" },
    ];

    const [lista, setLista] = useState(mock);
    const [selecionado, setSelecionado] = useState(null);

    const [dados, setDados] = useState({
        codigo: "",
        descricao: "",
    });

    /* 👉 AUTO INCREMENT */
    const proximoCodigo = () =>
        String(Math.max(...lista.map((x) => Number(x.codigo))) + 1).padStart(3, "0");

    /* 👉 Filtrar enquanto digita */
    const filtrados = lista.filter((item) =>
        item.descricao.toLowerCase().includes(dados.descricao.toLowerCase())
    );

    /* ------------ FUNÇÕES CRUD ------------ */
    const limpar = () => {
        setDados({ codigo: "", descricao: "" });
        setSelecionado(null);
    };

    const incluir = () => {
        if (!dados.descricao) {
            alert("Informe o nome do modelo!");
            return;
        }

        const novo = {
            codigo: proximoCodigo(),
            descricao: dados.descricao,
        };

        setLista([...lista, novo]);
        limpar();
    };

    const alterar = () => {
        if (selecionado === null) {
            alert("Selecione um registro!");
            return;
        }

        const nova = [...lista];
        nova[selecionado] = { ...dados };
        setLista(nova);
        limpar();
    };

    const excluir = () => {
        if (selecionado === null) {
            alert("Selecione um registro!");
            return;
        }

        setLista(lista.filter((_, idx) => idx !== selecionado));
        limpar();
    };

    const selecionar = (item, idx) => {
        setSelecionado(idx);
        setDados(item);
    };

    /* ENTER / TAB carrega o primeiro encontrado */
    const handleEnter = (e) => {
        if (e.key === "Enter" || e.key === "Tab") {
            const encontrado = lista.find(
                (x) =>
                    x.descricao.toLowerCase() === dados.descricao.toLowerCase()
            );

            if (encontrado) {
                const idx = lista.indexOf(encontrado);
                selecionar(encontrado, idx);
            }
        }
    };

    return (
        <div
            className={`transition-all duration-300 mt-[44px] text-[13px] text-gray-700 
      bg-gray-50 h-[calc(100vh-56px)] flex flex-col
      ${open ? "ml-[192px]" : "ml-[56px]"}`}
        >
            {/* TÍTULO */}
            <h1 className="text-center text-red-700 font-semibold py-1 text-sm border-b border-gray-300">
                CADASTRO MODELO DE VEÍCULO
            </h1>

            {/* CONTEÚDO */}
            <div className="flex-1 p-3 bg-white border-x border-b border-gray-300 
        rounded-b-md overflow-y-auto flex flex-col">

                {/* CARD GERAL */}
                <div className="border border-gray-300 rounded p-3 bg-white flex flex-col gap-3">

                    {/* CARD 1 - PARÂMETROS */}
                    <fieldset className="border border-gray-300 rounded p-3">
                        <legend className="px-2 text-red-700 font-semibold text-[13px]">
                            Parâmetros
                        </legend>

                        {/* Linha 1 */}
                        <div className="grid grid-cols-12 gap-2 mb-2">
                            <Label className="col-span-2">Código Modelo</Label>
                            <Txt
                                className="col-span-3 bg-gray-200"
                                readOnly
                                value={dados.codigo}
                            />
                        </div>

                        {/* Linha 2 */}
                        <div className="grid grid-cols-12 gap-2 mb-2">
                            <Label className="col-span-2">Nome Modelo</Label>
                            <Txt
                                className="col-span-6"
                                value={dados.descricao}
                                onChange={(e) =>
                                    setDados({ ...dados, descricao: e.target.value })
                                }
                                onKeyDown={handleEnter}
                            />
                        </div>
                    </fieldset>

                    {/* CARD 2 – GRID */}
                    <fieldset className="border border-gray-300 rounded p-3">
                        <legend className="px-2 text-red-700 font-semibold text-[13px]">
                            Modelos Cadastrados
                        </legend>

                        <div className="border border-gray-300 rounded max-h-[380px] overflow-y-auto">
                            <table className="w-full text-[12px]">
                                <thead className="bg-gray-100">
                                    <tr>
                                        <th className="border px-2 py-1">Código</th>
                                        <th className="border px-2 py-1">Descrição</th>
                                    </tr>
                                </thead>

                                <tbody>
                                    {filtrados.map((item, idx) => (
                                        <tr
                                            key={idx}
                                            onClick={() =>
                                                selecionar(item, lista.indexOf(item))
                                            }
                                            className={`cursor-pointer hover:bg-red-100 ${selecionado === lista.indexOf(item)
                                                    ? "bg-red-200"
                                                    : ""
                                                }`}
                                        >
                                            <td className="border px-2 py-1">{item.codigo}</td>
                                            <td className="border px-2 py-1">{item.descricao}</td>
                                        </tr>
                                    ))}

                                    {filtrados.length === 0 && (
                                        <tr>
                                            <td
                                                colSpan={2}
                                                className="text-center py-3 text-gray-500 border"
                                            >
                                                Nenhum modelo encontrado.
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    </fieldset>

                </div>
            </div>

            {/* ===== RODAPÉ PADRÃO ===== */}
            <div className="border-t border-gray-300 bg-white py-2 px-4 flex items-center gap-6">

                <button
                    onClick={() => navigate(-1)}
                    className={`flex flex-col items-center text-[11px] 
            ${footerIconColorNormal} hover:${footerIconColorHover}`}
                >
                    <XCircle size={20} />
                    <span>Fechar</span>
                </button>

                <button
                    onClick={limpar}
                    className={`flex flex-col items-center text-[11px] 
            ${footerIconColorNormal} hover:${footerIconColorHover}`}
                >
                    <RotateCcw size={20} />
                    <span>Limpar</span>
                </button>

                <button
                    onClick={incluir}
                    className={`flex flex-col items-center text-[11px] 
            ${footerIconColorNormal} hover:${footerIconColorHover}`}
                >
                    <PlusCircle size={20} />
                    <span>Incluir</span>
                </button>

                <button
                    onClick={alterar}
                    className={`flex flex-col items-center text-[11px] 
            ${footerIconColorNormal} hover:${footerIconColorHover}`}
                >
                    <Edit size={20} />
                    <span>Alterar</span>
                </button>

                <button
                    onClick={excluir}
                    className={`flex flex-col items-center text-[11px] 
            ${footerIconColorNormal} hover:${footerIconColorHover}`}
                >
                    <Trash2 size={20} />
                    <span>Excluir</span>
                </button>
            </div>
        </div>
    );
}
