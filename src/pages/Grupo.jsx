...
/* ========================= Mocks ========================= */
const gruposMockBase = [
    { codigo: "001", descricao: "ADMINISTRATIVO" },
    { codigo: "002", descricao: "INFORMATICA TI" },
    { codigo: "003", descricao: "OPERACIONAL" },
    { codigo: "004", descricao: "TESTE" },
];

export default function Grupo({ open }) {
    const location = useLocation();
    const isSeguranca = location.pathname.startsWith("/modulo-seguranca");
    const navigate = useNavigate();
    const { footerIconColorNormal, footerIconColorHover } = useIconColor();

    const [lista, setLista] = useState(gruposMockBase);
    const [dados, setDados] = useState({ codigo: "", descricao: "" });
    ...
    const limpar = () => { setDados({ codigo: "", descricao: "" }); setEditIndex(null); };

    const incluir = () => {
        if (!dados.codigo || !dados.descricao) {
            setModalMsg({ tipo: "info", texto: "Informe todos os campos!" }); return;
        }
        setModalMsg({
            tipo: "confirm", texto: "Confirma a Inclusão?",
            onYes: () => {
                setLista([...lista, dados]); limpar();
                setModalMsg({ tipo: "sucesso", texto: "Grupo incluído com sucesso!" });
            },
        });
    };
    ...
    const alterar = () => {
        ...
        const nova = [...lista]; nova[editIndex] = dados; setLista(nova); limpar();
        setModalMsg({ tipo: "sucesso", texto: "Grupo alterado com sucesso!" });
    };

    const excluir = () => {
        if (editIndex === null) { setModalMsg({ tipo: "info", texto: "Selecione um registro!" }); return; }
        setModalMsg({
            tipo: "confirm", texto: "Deseja excluir este grupo?",
            onYes: () => {
                setLista(lista.filter((_, i) => i !== editIndex)); limpar();
                setModalMsg({ tipo: "sucesso", texto: "Grupo excluído com sucesso!" });
            },
        });
    };
                ...
                <div className="border border-gray-300 rounded p-3 bg-white flex flex-col gap-3">
                    ...
                    <fieldset className="border border-gray-300 rounded p-3">
                        ...
                        <div className="grid grid-cols-12 gap-2 mb-2">
                            <Label className="col-span-1 justify-end">Código Grupo</Label>
                            <Txt
                        ...
                        <div className="grid grid-cols-12 gap-2 mb-2">
                            <Label className="col-span-1 justify-end">Nome Grupo</Label>
                            <Txt
                    ...
                    <fieldset className="border border-gray-300 rounded p-3">
                        <legend className="px-2 text-red-700 font-semibold text-[13px]">Registros Cadastrados</legend>
