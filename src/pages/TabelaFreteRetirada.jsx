...
export default function TabelaFreteRetirada({ onClose }) {
  ...
  const handleIncluir = () => {
    if (!dados.codigoLocal || !dados.descricao) {
      alert("Preencha o Local de Retirada Container!");
      return;
    }

    // Garante formatação correta da última taxa
    const formatado = dados.taxaDesova40.replace(",", ".").trim();
    const num = parseFloat(formatado);
    const taxaDesova40Fmt = isNaN(num)
      ? "0,00"
      : num.toFixed(2).replace(".", ",");

    setItens([...itens, { ...dados, taxaDesova40: taxaDesova40Fmt }]);
    setDados({
      codigoLocal: "",
      descricao: "",
      taxaColeta: "0,00",
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999]">
      <div
        ref={modalRef}
        className="absolute bg-white rounded-lg shadow-xl w-[90%] max-w-[1200px] max-h-[90vh] overflow-auto border border-gray-300 text-black"
      ...
      >
        ...
        <div className="p-4 space-y-3">
          ...
          <fieldset className="border border-gray-300 rounded p-3">
            ...
            {/* Demais linhas */}
            <div className="grid grid-cols-3 gap-x-6 gap-y-[6px] text-[13px]">
              {[
                ["Taxa de Coleta", "taxaColeta"],
                ["Valor Pedágio por Eixo", "valorPedagioEixo"],
                ["Taxa de Ova 20'", "taxaOva20"],
                ["Taxa de Ova 40'", "taxaOva40"],
                ["Taxa de Desova 20'", "taxaDesova20"],
                ["Taxa de Desova 40'", "taxaDesova40"],
              ].map(([label, name], i) => (
                <div key={i} className="flex items-center gap-2">
                  <label className="w-[160px] text-right">{label}</label>
                  <input
                    name={name}
                    type="text"
                    inputMode="decimal"
                    ...
                    onBlur={(e) => {
                      handleBlur(e);
                      if (name === "taxaDesova40") handleUltimoCampo();
                    }}
                    onKeyDown={(e) => {
                      if (name === "taxaDesova40" && e.key === "Enter") {
                        e.preventDefault();
                        handleBlur(e);
                        handleUltimoCampo();
                      }
            ...
            <table className="min-w-full border-collapse text-[13px]">
              <thead className="bg-gray-200 text-gray-800 font-semibold text-center">
                <tr>
                  ...
                  <th className="border border-gray-300 px-2 py-1">Taxa Ova 20'</th>
                  <th className="border border-gray-300 px-2 py-1">Taxa Ova 40'</th>
                  <th className="border border-gray-300 px-2 py-1">
                    Taxa Desova 20'
                  </th>
                  <th className="border border-gray-300 px-2 py-1">
